...
OM_JUCE_API void* makeAudioSourceFromBuffer (float** audio_buffer, int numChannels, int numSamples, int sampleRate);
...
OM_JUCE_API bool getAudioFileSamples (void* filereader, float** buffer, long long startPos, int n_samples);

OM_JUCE_API void* makeAudioFileWriter (const char* path, int format);
OM_JUCE_API void freeAudioFileWriter (void* filewriter);
OM_JUCE_API bool writeSamplesToAudioFile (void* filewriter, float** buffer, int n_channels, long long size, double sr, int ss);
